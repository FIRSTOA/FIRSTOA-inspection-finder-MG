...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!/^01\d{8,9}$/.test(to) || !text) return Response.json({ error: "수신번호 또는 메시지가 올바르지 않습니다." }, { status: 400, headers: corsHeaders });

    const response = await fetch(webhookUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...body, to, text, source: "FIRSTOA_CS_SYSTEM" }),
    });
