...
// 워킨맵(현 분기 동선 데이터)으로 업체명을 정하던 findWorkinMapName은 걷어냈다(2026-08-26).
// 접수 업체의 30%는 워킨맵에 아예 없고(현 분기 동선 기준 36%), 있어도 회사명만으로는 본사·현장을 가릴 수 없어
// "드림엔지니어링본사"·"세안이엔씨삼성서울병원"처럼 엉뚱한 지점이 양식에 적혔다. 업체명 원본은 임대리스트 시트 원문(CA열) — src/vendorName.ts.
...
export async function saveServiceReception(row: Omit<ServiceReceptionRow, "id" | "created_at" | "receipt_date">): Promise<string> {
  ...
  // 새 접수 웹푸시 — 지역의 팀 글자(수도권A→A팀)가 있으면 그 팀에게만, 못 읽으면 전체(접수자 본인 제외)
  const receptionTeam = normRegion(row.region || "");
  void invokeEdgeFunction("push-send", {
    ...
    category: "reception",
    ...(receptionTeam ? { team: receptionTeam } : { all: true }),
...
const REGION_ROOMS: Record<string, Record<string, string>> = {
  ...
  recontract: {
    ...
    C: "강남C/초과사용 계약종료체크",
    D: "경기D/초과사용 계약종료체크",
    E: "충청외극지방 E/초과사용 계약종료체크", // 지방 초과방 (2026-09-17) — 미수·불만 E방은 이름 확인 전이라 비움
  },
};
