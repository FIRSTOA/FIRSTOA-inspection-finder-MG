...
export default function ReportTypeSelector({ selected, other, onSelected, onOther, accent = "#334155" }: {
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-50 flex items-end bg-black/40" onClick={() => setOpen(false)} role="dialog">
      <div className="w-full rounded-t-2xl bg-white shadow-xl sm:rounded-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3"><div><div className="text-sm font-bold text-slate-800">구분 선택</div><div className="text-[11px] text-slate-400">점검과 AS처럼 여러 개를 함께 선택할 수 있습니다.</div></div><button type="button" onClick={() => setOpen(false)} className="rounded-full px-3 py-1.5 text-xs font-bold text-slate-500 transition hover:bg-slate-100 hover:text-slate-700">닫기</button></div>
        ...
        {selected.includes("기타") && <div className="px-4 pb-3"><input value={other} onChange={(e) => onOther(e.target.value)} placeholder="기타 구분 직접입력" className="w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-base outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" /></div>}
