...
const initialPlaces: MapPlace[] = [
  ...
  { id: 2, number: 2, label: "G3", visible: true, name: "11SO 이크럭스벤처파트너스(유) 분기마감", comment: "APEOSPORT-C2060 / 227683", phone: "박담당 010-0000-3333", address: "서울 강남구 강남대로 320", addressDetail: "5층 종합회의실", latitude: 37.4918, longitude: 127.0311, memos: ["방문주기 3개월", "계약종료년월 2606", "강남", "S"] },
...
export default function WalkingMap({ userKey = "guest", onSelfRequest }: { userKey?: string; onSelfRequest?: (text: string) => void }) {
                ...
                <span className="min-w-0">
                  ...
                  {((place.kind === "quarter" && renewalMatch) || misu || overage || bulman) && (
                    <span className="mt-1 flex flex-wrap gap-1">
                      ...
                      {misu && <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-black text-amber-700">{(misuMonths || misuBal) ? `미수 ${misuMonths ? `${misuMonths}개월` : ""}${misuMonths && misuBal ? " · " : ""}${misuBal}` : "미수 확인필요"}</span>}
                      {overage && <span className="rounded-full bg-purple-50 px-2 py-0.5 text-[10px] font-black text-purple-700">초과 {misuBalanceLabel(overage.total)}{overage.date ? ` (${overage.date.slice(2, 7)})` : ""}</span>}
                      {bulman && <span className="rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-black text-red-700" title={bulman.content}>불만 {bulman.date.slice(2, 4)}년 {Number(bulman.date.slice(5, 7))}월{bulman.content ? ` · ${bulman.content.slice(0, 14)}` : ""}</span>}
                    </span>
                  )}
                </span>
              </button>
  ...
  /**
   * 상단 다크바(모바일) — 왼쪽엔 지금 보고 있는 조건(C 3Q 분기점검), 오른쪽 끝엔 검색 버튼.
   * 지도 조작 버튼(내 위치·조건·색상·진행률)은 지도 우측 상단에 두고, 헤더는 "지금 무엇을 보는지"만 알려준다.
   */
  ...
  const mapPanel = (
    <div className="relative h-full min-h-0 overflow-hidden bg-slate-100 lg:min-h-[540px]">
      ...
      {/* 지도 컨트롤 — 우측 상단에 쫙 붙인다(모바일 요청). 패널은 이 묶음 아래로 우측 정렬로 열린다. */}
      <div className="absolute right-2 top-2 z-[1000] lg:right-3 lg:top-3">
        <div className="relative flex justify-end gap-1">
          ...
          {colorMenuOpen && (
            <div className="absolute right-0 top-11 z-[1200] max-h-[calc(100dvh-11rem)] w-[330px] max-w-[calc(100vw-1rem)] overflow-y-auto overscroll-contain rounded-xl border border-slate-200 bg-white p-3 shadow-2xl">
          ...
          {progressMenuOpen && (
            <div className="absolute right-0 top-11 z-[1200] max-h-[calc(100dvh-11rem)] w-[370px] max-w-[calc(100vw-1rem)] overflow-y-auto overscroll-contain rounded-xl border border-slate-200 bg-white p-4 pb-6 shadow-2xl">
