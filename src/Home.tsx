...
export default function Home({ onGoField }: { onGoField: () => void }) {
  const cats: { icon: string; name: string; tone: string }[] = [
    { icon: "🔧", name: "점검", tone: "#EFF6FF" },
    { icon: "🛠", name: "AS", tone: "#EFF6FF" },
  ...
  return (
    <div className="space-y-4 pb-10">
      {/* 메인 박스 */}
      <div className="rounded-3xl bg-gradient-to-br from-[#3182F6] to-[#1B64DA] p-6 text-white shadow-lg shadow-blue-500/20">
        ...
        <button onClick={onGoField} className="mt-5 w-full rounded-2xl bg-white py-3.5 text-sm font-bold text-[#3182F6] transition active:scale-[0.98]">
          FIELD 시작하기 →
        </button>
      </div>
      ...
      <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
        ...
        <div className="grid grid-cols-4 gap-2">
          ...
          <button onClick={onGoField} className="flex flex-col items-center justify-center gap-1.5 rounded-xl py-2.5 text-[#3182F6]">
            <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#3182F6] text-xl text-white">→</span>
      ...
      <Section title="상단 아이콘" sub="화면 맨 위 버튼들" defaultOpen>
        ...
        <Row icon="🔍" tone="#EFF6FF" name="거래처검색">거래처명으로 지난 점검/AS 양식을 찾아 불러와요. 결과는 <b>지역(A~E)·계열</b>로 묶여 보이고, 확장성 탭에선 청정기/PC 기록을 검색해요.</Row>
        <Row icon="📝" tone="#EFF6FF" name="원본입력">카톡 원본 글을 붙여넣으면 자동으로 양식으로 변환돼요. (점검/AS)</Row>
        <Row icon="📷" tone="#EFF6FF" name="사진양식">모바일 <b>워킹맵 캡처 사진</b>을 올리면 AI가 점검/청정기 양식을 자동으로 만들어줘요.</Row>
        <Row icon="🗂️" tone="#EFF6FF" name="통합이력">그 거래처의 점검·AS·초과·미수·불만·재계약 등 <b>전체 이력</b>을 한눈에 봐요.</Row>
      </Section>

      {/* 탭 안내 */}
      <Section title="탭 안내" sub="점검 / AS / 확장성 / 더보기">
        <Row icon="🔧" tone="#EFF6FF" name="점검">복합기/프린터 점검. 위 토글로 <b>복합기 ↔ 청정기</b> 전환. 거래처검색·사진양식 지원.</Row>
        <Row icon="🛠" tone="#EFF6FF" name="AS">AS 접수내용을 붙여넣으면 깔끔한 양식으로 변환돼요.</Row>
      ...
      <Section title="보내기 · 자가 · 부품" sub="맨 아래 버튼 (실제 모양)">
        <BtnRow chip={<span className={chipBase} style={{ background: "#3182F6", color: "#fff" }}>보내기</span>}>
      ...
      <Section title="사진 첨부 · 불러오기" sub="현장에서 유용한 기능">
        <Row icon="📷" tone="#EFF6FF" name="사진 첨부 (폼 하단)">현장 사진을 여러 장(수십 장도) 선택 → 보내기 하면 <b>링크 1개</b>로 모아져 카톡에 전송돼요. 링크 누르면 사진 갤러리.</Row>
