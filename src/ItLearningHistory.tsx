...
export default function ItLearningHistory({ author }: { author: string }) {
  ...
  return <div className="space-y-4">
    ...
    {connectionOpen && <section className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3"><div className="mx-auto flex max-w-3xl flex-col gap-2 sm:flex-row"><input value={endpointDraft} onChange={(event) => setEndpointDraft(event.target.value)} placeholder="Apps Script 웹 앱 /exec 주소" className="h-10 min-w-0 flex-1 rounded-full border border-slate-300 bg-white transition hover:bg-slate-50 px-3 text-sm outline-none focus:border-blue-500" /><button type="button" onClick={() => void saveEndpoint()} className="h-10 rounded-full bg-slate-900 transition hover:bg-slate-800 px-4 text-sm font-black text-white">연결 확인</button></div><p className="mx-auto mt-2 max-w-3xl text-[11px] font-semibold text-slate-400">기존 PC DB Apps Script에 API 어댑터를 적용한 뒤 배포 주소를 한 번만 입력합니다.</p></section>}
