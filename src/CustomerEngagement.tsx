  ...
  return <div className="space-y-2">
    {contacts.map((contact, index) => <div key={contact.id} className={`grid grid-cols-[24px_minmax(0,1fr)_32px] items-start gap-2 rounded-md border p-2 ${contact.selected ? "border-blue-200 bg-blue-50/50" : "border-slate-200 bg-slate-50 opacity-60"}`}>
      ...
      <div className="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2">
        ...
        {email && <input type="email" value={contact.email} onChange={(event) => patch(contact.id, { email: event.target.value })} placeholder="이메일" className="min-w-0 rounded border border-slate-200 bg-white px-2 py-2 text-xs sm:col-span-2" />}
      </div>
...
export function HappyCallWorkspace({ author }: { author: string }) {
  ...
  const detail = selected ? <div className="flex min-h-0 flex-col">
    ...
    <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3"><label className="text-xs font-black text-slate-500">예약 발송 시간<input type="datetime-local" value={scheduleAt} onChange={(event) => setScheduleAt(event.target.value)} className="mt-1 w-full rounded-full border border-slate-300 bg-white transition hover:bg-slate-50 px-3 py-2 text-sm" /></label></div>
    ...
    <div className="sticky bottom-0 -mx-4 mt-4 grid grid-cols-2 gap-2 border-t bg-white/95 p-3 backdrop-blur sm:flex sm:flex-wrap xl:static xl:mx-0 xl:p-0 xl:pt-4">
      ...
      {selectedRecord?.status === "scheduled" && <button onClick={() => void cancelSchedule()} className="rounded-md border border-rose-200 bg-rose-50 px-3 py-2.5 text-sm font-black text-rose-600">예약 취소</button>}
      <button disabled={sending} onClick={() => void schedule()} className="rounded-md border border-blue-200 px-4 py-2.5 text-sm font-black text-blue-700 disabled:opacity-50 sm:ml-auto">예약 발송</button>
  ...
  return <div className="grid gap-4 xl:grid-cols-[minmax(0,1.05fr)_minmax(430px,.95fr)]">
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 p-4 sm:p-5"><div className="flex items-end justify-between gap-3"><div><h2 className="text-xl font-black">해피콜</h2><p className="mt-1 text-sm font-semibold text-slate-500">최근 7일 방문기록</p></div></div>
        <div className="mt-4 grid grid-cols-5 rounded-full bg-slate-100 p-1">{(["pending", "scheduled", "cancelled", "sent", "all"] as const).map((item) => <button key={item} onClick={() => setFilter(item)} className={`rounded px-1 py-2 text-[11px] font-black sm:text-xs ${filter === item ? "bg-white shadow" : "text-slate-500"}`}>{item === "pending" ? "대기" : item === "scheduled" ? "예약" : item === "cancelled" ? "예약취소" : item === "sent" ? "완료" : "전체"}</button>)}</div>
        <div className="mt-2 grid grid-cols-2 rounded-full bg-slate-100 p-1"><button onClick={() => { setKindFilter("inspection"); setSelectedId(""); }} className={`rounded py-2 text-sm font-black ${kindFilter === "inspection" ? "bg-white text-blue-700 shadow" : "text-slate-500"}`}>점검</button><button onClick={() => { setKindFilter("as"); setSelectedId(""); }} className={`rounded py-2 text-sm font-black ${kindFilter === "as" ? "bg-white text-rose-600 shadow" : "text-slate-500"}`}>AS</button></div>
      </div>
...
export function PromoWorkspace({ author }: { author: string }) {
  ...
  const detail = selected ? <div className="flex min-h-0 flex-col">
    <div className="flex items-start justify-between gap-3"><div><div className="text-xs font-black text-blue-600">{selected.category}</div><div className="mt-1 text-lg font-black">{selected.title}</div></div><button onClick={() => void removeMaterial()} className="rounded-md border border-rose-200 px-3 py-2 text-xs font-black text-rose-600">삭제</button></div>
    <div className="mt-3 aspect-[16/9] overflow-hidden rounded-md border bg-slate-100"><MaterialPreview material={selected} /></div>
    <div className="mt-4 text-xs font-black text-slate-500">
      ...
      {visitPickerOpen && <div className="mt-2 max-h-56 overflow-y-auto rounded-md border border-slate-200 bg-white p-1 shadow-sm">
