...
function pdfPreviewUrl(url: string) { return `https://docs.google.com/gview?embedded=1&url=${encodeURIComponent(url)}`; }
...
function TemplateBar({ context, author, body, onApply }: { context: "happycall" | "promotion"; author: string; body: string; onApply: (body: string) => void }) {
  ...
  return <div className="flex flex-wrap gap-2">
    <select value={selectedId} onChange={(event) => { setSelected(event.target.value); const template = templates.find((item) => item.id === event.target.value); if (template) onApply(template.body); }} className="min-w-0 flex-1 rounded-md border border-slate-300 bg-white px-3 py-2 text-xs font-black">
      {templates.map((template) => <option key={template.id} value={template.id}>{template.title}</option>)}
    </select>
    <button type="button" onClick={() => void save(body)} className="rounded-md border border-blue-200 px-3 py-2 text-xs font-black text-blue-600">새 문구 추가</button>
...
export function HappyCallWorkspace({ author }: { author: string }) {
  ...
  return <div className="grid gap-4 xl:grid-cols-[minmax(0,1.05fr)_minmax(430px,.95fr)]">
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-xl font-black">해피콜 대기</h2><p className="mt-1 text-sm font-semibold text-slate-500">최근 7일 방문기록</p></div><div className="rounded-md bg-slate-100 p-1">{(["pending", "scheduled", "cancelled", "sent", "all"] as const).map((item) => <button key={item} onClick={() => setFilter(item)} className={`rounded px-3 py-1.5 text-xs font-black ${filter === item ? "bg-white shadow" : "text-slate-500"}`}>{item === "pending" ? "대기" : item === "scheduled" ? "예약" : item === "cancelled" ? "예약취소" : item === "sent" ? "완료" : "전체"}</button>)}</div></div><div className="mt-4 grid grid-cols-2 rounded-md bg-slate-100 p-1"><button onClick={() => { setKindFilter("inspection"); setSelectedId(""); }} className={`rounded py-2 text-sm font-black ${kindFilter === "inspection" ? "bg-white text-blue-700 shadow" : "text-slate-500"}`}>점검 해피콜</button><button onClick={() => { setKindFilter("as"); setSelectedId(""); }} className={`rounded py-2 text-sm font-black ${kindFilter === "as" ? "bg-white text-rose-600 shadow" : "text-slate-500"}`}>AS 해피콜</button></div></div><div className="max-h-[700px] divide-y overflow-y-auto">{rows.map((visit) => { const status = recordMap.get(visit.id)?.status || "pending"; return <button key={visit.id} onClick={() => choose(visit)} className={`block w-full p-4 text-left hover:bg-slate-50 ${selectedId === visit.id ? "bg-blue-50" : ""}`}><div className="flex justify-between gap-3"><div><div className="text-sm font-black">{visit.vendor}</div><div className="mt-1 text-xs font-semibold text-slate-500">{visit.workDate} · {visitType(visit)} · {visit.author}</div></div><span className="shrink-0 text-[10px] font-black text-blue-600">{statusLabel(status)}</span></div></button>; })}{!loading && !rows.length && <div className="p-12 text-center text-sm text-slate-400">방문기록이 없습니다.</div>}</div></section>
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">{!selected ? <div className="flex min-h-[430px] items-center justify-center text-center text-sm font-semibold text-slate-400">방문 건을 선택하세요.</div> : <div><div className="border-b pb-4"><div className="text-xs font-black text-blue-600">{selected.workDate} · {visitType(selected)}</div><div className="mt-1 text-lg font-black">{selected.vendor}</div></div><div className="mt-4"><div className="mb-2 text-xs font-black text-slate-500">발송 대상</div><ContactsEditor contacts={contacts} onChange={setContacts} /></div><div className="mt-4"><TemplateBar context="happycall" author={author} body={message} onApply={setMessage} /></div><textarea rows={8} value={message} onChange={(event) => setMessage(event.target.value)} className="mt-2 w-full rounded-md border border-slate-300 p-3 text-sm leading-6" /><div className="text-[10px] font-bold text-slate-400">사용 가능: {'{고객명}'} {'{업체명}'} {'{담당자}'} {'{업무}'} {'{방문일}'}</div><div className="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3"><label className="text-xs font-black text-slate-500">예약 발송 시간<input type="datetime-local" value={scheduleAt} onChange={(event) => setScheduleAt(event.target.value)} className="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm" /></label></div>{notice && <div className="mt-3 rounded-md bg-slate-100 p-3 text-xs font-bold text-slate-600">{notice}</div>}<div className="mt-4 flex flex-wrap gap-2"><button onClick={() => void saveRecord("skip")} className="rounded-md border px-3 py-2.5 text-sm font-black text-slate-500">대상 제외</button>{selectedRecord?.status === "scheduled" && <button onClick={() => void cancelSchedule()} className="rounded-md border border-rose-200 bg-rose-50 px-3 py-2.5 text-sm font-black text-rose-600">예약 취소</button>}<button disabled={sending} onClick={() => void schedule()} className="ml-auto rounded-md border border-blue-200 px-4 py-2.5 text-sm font-black text-blue-700 disabled:opacity-50">예약 발송</button><button disabled={sending} onClick={() => void send()} className="rounded-md bg-blue-600 px-4 py-2.5 text-sm font-black text-white disabled:opacity-50">{sending ? "처리 중" : "지금 발송"}</button></div></div>}</section>
  </div>;
}
