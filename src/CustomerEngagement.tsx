...
export function PromoWorkspace({ author }: { author: string }) {
  ...
  const detail = selected ? <div className="flex min-h-0 flex-col">
    ...
    <div className="mt-4 text-xs font-black text-slate-500">
      최근 방문 업체
      <button type="button" onClick={() => setVisitPickerOpen((current) => !current)} className="mt-1 flex w-full items-center justify-between gap-3 rounded-md border border-slate-300 bg-white px-3 py-2.5 text-left text-sm font-bold text-slate-800">
