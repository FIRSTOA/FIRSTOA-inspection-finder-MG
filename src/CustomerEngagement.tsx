...
export function HappyCallWorkspace({ author }: { author: string }) {
  ...
  const detail = selected ? <div className="flex min-h-0 flex-col">
    ...
    <div className="sticky bottom-0 -mx-4 mt-4 grid grid-cols-2 gap-2 border-t bg-white/95 p-3 backdrop-blur sm:flex sm:flex-wrap xl:static xl:mx-0 xl:p-0 xl:pt-4">
      ...
      <button disabled={sending} onClick={() => void send()} className="rounded-md bg-blue-600 px-4 py-2.5 text-sm font-black text-white disabled:opacity-50">{sending ? "처리 중" : "지금 발송"}</button>
    </div>
  ...
  return <div className="grid gap-4 xl:grid-cols-[minmax(0,1.05fr)_minmax(430px,.95fr)]">
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 p-4 sm:p-5"><div className="flex items-end justify-between gap-3"><div><h2 className="text-xl font-black">해피콜</h2><p className="mt-1 text-sm font-semibold text-slate-500">최근 7일 방문기록</p></div></div>
        <div className="mt-4 grid grid-cols-5 rounded-md bg-slate-100 p-1">{(["pending", "scheduled", "cancelled", "sent", "all"] as const).map((item) => <button key={item} onClick={() => setFilter(item)} className={`rounded px-1 py-2 text-[11px] font-black sm:text-xs ${filter === item ? "bg-white shadow" : "text-slate-500"}`}>{item === "pending" ? "대기" : item === "scheduled" ? "예약" : item === "cancelled" ? "예약취소" : item === "sent" ? "완료" : "전체"}</button>)}</div>
        <div className="mt-2 grid grid-cols-2 rounded-md bg-slate-100 p-1"><button onClick={() => { setKindFilter("inspection"); setSelectedId(""); }} className={`rounded py-2 text-sm font-black ${kindFilter === "inspection" ? "bg-white text-blue-700 shadow" : "text-slate-500"}`}>점검</button><button onClick={() => { setKindFilter("as"); setSelectedId(""); }} className={`rounded py-2 text-sm font-black ${kindFilter === "as" ? "bg-white text-rose-600 shadow" : "text-slate-500"}`}>AS</button></div>
      </div>
...
export function PromoWorkspace({ author }: { author: string }) {
  ...
  const detail = selected ? <div className="flex min-h-0 flex-col">
    ...
    <label className="mt-4 block text-xs font-black text-slate-500">최근 방문 업체<select value={sourceVisitId} onChange={(event) => chooseVisit(event.target.value)} className="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2.5 text-sm"><option value="">직접 입력</option>{visits.map((visit) => <option key={visit.id} value={visit.id}>{visit.workDate} · {visit.vendor} · {visitType(visit)}</option>)}</select></label>
