...
export function PromoWorkspace({ author }: { author: string }) {
  ...
  const detail = selected ? <div className="flex min-h-0 flex-col">
    ...
    {notice && <div className="mt-3 rounded-md bg-slate-100 p-3 text-xs font-bold text-slate-600">{notice}</div>}
    <div className="sticky bottom-0 -mx-4 mt-4 grid grid-cols-2 gap-2 border-t bg-white/95 p-3 backdrop-blur xl:static xl:mx-0 xl:p-0 xl:pt-4"><button onClick={() => void send("sms")} className="rounded-full bg-blue-600 shadow-[0_3px_10px_rgba(37,99,235,0.3)] hover:bg-blue-700 px-3 py-2.5 text-sm font-black text-white">문자 발송</button><button onClick={() => void send("email")} className="rounded-md border border-blue-200 px-3 py-2.5 text-sm font-black text-blue-700">메일 발송</button><button type="button" onClick={() => window.open(selected.file_url, "_blank", "noopener,noreferrer")} className="rounded-md border px-3 py-2.5 text-center text-sm font-black">원본 열기</button><a href={downloadUrl(selected.file_url, selected.title)} className="rounded-md border px-3 py-2.5 text-center text-sm font-black">파일 저장</a></div>
