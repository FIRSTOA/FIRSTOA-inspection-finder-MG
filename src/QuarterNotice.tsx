...
function QuarterNoticeBoard({ author, switcher }: { author: string; switcher?: ReactNode }) {
  ...
  }, [places, grades, team, excludeDone, excludeSent, sentPhones]);
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_380px]">
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/70 px-4 py-2.5">
            <span className="text-sm font-black text-slate-900">발송 대상 <span className="text-blue-600">{picked.length}</span>/{targets.length}곳 <span className="ml-1 text-[11px] font-bold text-slate-400">(같은 번호는 1통으로 묶음)</span></span>
        ...
        <section className="space-y-3">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-2 text-[11px] font-semibold leading-4 text-slate-400">발송 즉시 대표번호로 나갑니다. 같은 분기에 보낸 번호는 기록되어 다음에 자동 제외됩니다.</div>
          </div>
        </section>
      </div>
