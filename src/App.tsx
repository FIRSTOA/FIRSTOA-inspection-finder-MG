...
const TEST_CASES: TestCase[] = [
  ...
  },
  {
    name: "위치 제목 자동 번호 부여",
    input: "-------------------------------------\n15층입구\n모델명: D470\n시리얼넘버: 809150608947",
    // FIELD 현행 형식: 번호 줄("1.") 아래에 위치가 따로 온다 — c0caf6d에서 바뀐 뒤 기대값만 옛 한 줄 형식으로 남아 있었다
    expected: "1.\n15층입구",
    mode: "inspection",
  },
  {
...
export default function App() {
  ...
  const handlePreviewBlockChange = (block: ResultBlock, index: number, value: string) => {
    ...
    if (authorValue) handleSetAuthor(authorValue); // 빈 값으로 작성자를 지우지 않는다
  ...
  const finishTicket = async (ticket: { id: string; receptionId: string; sentText?: string }, patch: Record<string, unknown>, receptionStatus: string, deferReason = "") => {
    try {
      ...
      if (reasonBlock) {
        // 사유는 양식이 간 방으로 — 양식의 "지역" 값을 우선한다(작성자 팀과 다른 지역을 골랐을 때 양식은 E방, 사유는 C방으로 갈라지던 것 — 2026-09-18). 없으면 일정의 팀
        const formRegion = regionLetter(String(ticket.sentText || "").match(/^\s*지역\s*[:：]\s*([^\n\r]+)/m)?.[1] || "");
        void sendServiceReception("AS", `수도권${formRegion || String(rows[0]?.["team"] || "")}`, reasonBlock)
      ...
      <div className={`mx-auto flex flex-col transition-[margin] duration-200 ${screen === "walkingMap" ? "px-0 pt-0 sm:px-0 sm:pt-0 lg:px-0 lg:pt-0" : "px-3 pt-4 sm:px-6 sm:pt-6"} ${screen !== "field" ? `max-w-[1800px] ${screen === "walkingMap" ? "pb-0 lg:max-w-none lg:px-0" : "pb-16 lg:px-8"} ${sidebarCollapsed ? "lg:ml-20" : "lg:ml-64"}` : `max-w-3xl lg:max-w-none lg:px-8 ${sidebarCollapsed ? "lg:ml-20" : "lg:ml-64"}`} ${screen === "field" && hasOutput && !previewCollapsed ? "pb-[46vh] lg:pb-8" : screen !== "field" ? "" : "pb-60"}`}>
          ...
          <div className="fixed inset-0 z-[300] flex items-end bg-black/40 sm:items-center sm:justify-center sm:p-4" onMouseDown={() => setTicketDonePrompt(null)}>
            <div className="w-full rounded-t-2xl bg-white p-5 shadow-xl sm:max-w-sm sm:rounded-xl" onMouseDown={(e) => e.stopPropagation()}>
              ...
              <div className="mt-2 rounded-lg bg-emerald-50 px-3 py-2 text-[12px] font-bold leading-5 text-emerald-800">
                ...
                <b>→ 익일로</b> → 방금 보낸 양식은 그대로 두고, 사유를 <b>두 번째 메시지</b>로 같은 AS방에 보냅니다. 네이버 일정은 날짜가 옮겨지고 양식+사유가 기록됩니다
                </>}
        ...
        <aside className="sticky top-5 hidden max-h-[calc(100vh-2.5rem)] min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:flex lg:flex-col">
          ...
          <div className="space-y-2 border-t border-slate-200 bg-slate-50 p-3">
            {(mode === "inspection" || mode === "blank-report") && currentVendor && <KeymanCard vendor={currentVendor} author={author} region={sharedForm.region} />}
