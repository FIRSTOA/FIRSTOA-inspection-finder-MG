    ...
    {!loading && <>
      ...
      {kind === "daily" && period === "day" ? <div className="space-y-6"><div className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(420px,1fr)]">
        ...
        <section className="h-fit rounded-xl border border-slate-200 bg-white p-5 xl:sticky xl:top-6"><div className="flex items-center justify-between"><div><h3 className="text-base font-black text-slate-950 lg:text-lg">내근 업무 입력</h3><p className="mt-0.5 text-xs font-semibold text-slate-400">수량·건수와 시간을 입력하세요.</p></div><label className="text-xs text-slate-500">복귀시간<input type="time" value={office.returnTime} onChange={(e) => setOffice({ ...office, returnTime: e.target.value })} className="ml-2 rounded-lg border border-slate-300 px-2 py-1.5 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" /></label></div>
          <div className="mt-4 divide-y divide-slate-100">{OFFICE_KINDS.map((k) => <div key={k} className="grid grid-cols-[1fr_90px_100px] items-center gap-2 py-2.5"><div className="text-sm font-semibold text-slate-700">{OFFICE_LABELS[k]}</div><label className="text-[10px] text-slate-400">수량/건<input type="number" min="0" value={office.values[k].count || ""} onChange={(e) => setOfficeValue(k, "count", Number(e.target.value))} className="mt-0.5 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" /></label><label className="text-[10px] text-slate-400">시간(분)<input type="number" min="0" value={office.values[k].minutes || ""} onChange={(e) => setOfficeValue(k, "minutes", Number(e.target.value))} className="mt-0.5 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" /></label></div>)}</div>
        ...
        <div className="grid gap-3 xl:grid-cols-3 2xl:grid-cols-6">
          {note.bottlenecks.map((item, i) => (
            <div key={i} className="rounded-lg border border-rose-100 bg-white p-4 shadow-sm">
              ...
              <label className="block text-xs font-black text-slate-500">병목현상<input value={item.title} onChange={(e) => onBottleneckChange(i, "title", e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-800 outline-none focus:border-rose-300" /></label>
              <label className="mt-3 block text-xs font-black text-slate-500">원인<AutoGrowTextarea value={item.cause} onChange={(value) => onBottleneckChange(i, "cause", value)} rows={1} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm leading-6 text-slate-700 outline-none focus:border-rose-300" /></label>
...
function PeriodBreakdown({ period, rows, officeLogs, start, end, year, month, quarter }: { period: Period; rows: VisitRow[]; officeLogs: OfficeLog[]; start: string; end: string; year: number; month: number; quarter: number }) {
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="mb-4"><h3 className="text-base font-black text-slate-950 lg:text-lg">{title}</h3><p className="mt-0.5 text-xs font-semibold text-slate-400">{desc}</p></div>
