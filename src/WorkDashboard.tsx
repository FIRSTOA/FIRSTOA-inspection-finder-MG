...
function PeriodBreakdown({ period, rows, officeLogs, start, end, year, month, quarter }: { period: Period; rows: VisitRow[]; officeLogs: OfficeLog[]; start: string; end: string; year: number; month: number; quarter: number }) {
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="mb-4"><h3 className="text-base font-black text-slate-950 lg:text-lg">{title}</h3><p className="mt-0.5 text-xs font-semibold text-slate-400">{desc}</p></div>
    ...
    <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[900px] text-left"><thead><tr className="border-b border-slate-200 text-[11px] font-black tracking-wide text-slate-400"><th className="px-3 py-3">구분</th><th className="px-3 py-3 text-right">방문</th><th className="px-3 py-3 text-right">기기</th><th className="px-3 py-3 text-right">점검</th><th className="px-3 py-3 text-right">AS</th><th className="px-3 py-3 text-right">납품</th><th className="px-3 py-3 text-right">PC·IT</th><th className="px-3 py-3 text-right">미수</th><th className="px-3 py-3 text-right">불만</th><th className="px-3 py-3 text-right">외근시간</th><th className="px-3 py-3 text-right">내근시간</th></tr></thead><tbody>{summaries.map(({ bucket, summary, inside }) => <tr key={`${bucket.start}-${bucket.end}`} className="border-b border-slate-100 last:border-0 hover:bg-slate-50"><td className="px-3 py-3.5 font-black text-slate-900">{bucket.label} {bucket.sub && <span className="ml-1 text-[10px] font-normal text-slate-400">{bucket.sub}</span>}</td><td className="px-3 py-3.5 text-right font-black tabular-nums text-slate-900">{summary.visits}</td><td className="px-3 py-3.5 text-right font-semibold tabular-nums text-slate-600">{summary.machines}</td><td className="px-3 py-3.5 text-right font-semibold tabular-nums text-slate-600">{summary.count.inspection}</td><td className="px-3 py-3.5 text-right font-semibold tabular-nums text-slate-600">{summary.count.as}</td><td className="px-3 py-3.5 text-right font-semibold tabular-nums text-slate-600">{summary.count.delivery}</td><td className="px-3 py-3.5 text-right font-semibold tabular-nums text-slate-600">{summary.count.pc}</td><td className="px-3 py-3.5 text-right font-semibold tabular-nums text-slate-600">{summary.count.misu}</td><td className="px-3 py-3.5 text-right font-semibold tabular-nums text-slate-600">{summary.count.bulman}</td><td className="px-3 py-3.5 text-right font-black tabular-nums text-blue-700">{hm(summary.fieldMinutes)}</td><td className="px-3 py-3.5 text-right font-black tabular-nums text-emerald-700">{hm(inside)}</td></tr>)}</tbody></table></div>
  </section>;
}
...
function HierarchicalVisitList({ period, rows, year, month, quarter, start, end }: { period: Period; rows: VisitRow[]; year: number; month: number; quarter: number; start: string; end: string }) {
  ...
  const summaryChips = (list: VisitRow[]) => {
    ...
    return <div className="flex flex-wrap gap-1.5">{items.map(([label, value]) => <span key={label} className="rounded-full border border-slate-200 bg-white px-2.5 py-0.5 text-[11px] font-bold tabular-nums text-slate-600"><b className="mr-1 font-black text-slate-400">{label}</b>{value}</span>)}</div>;
  };
  const visitRows = (list: VisitRow[]) => <div className="divide-y divide-slate-100">{list.map((r) => <div key={r.id} className="grid gap-2 px-5 py-3 md:grid-cols-[90px_1fr_auto] md:items-center"><div className="text-xs font-semibold text-slate-400">{r.arrivalTime || "시간 미입력"}</div><div><div className="text-sm font-black text-slate-900">{r.vendor} {r.grade && <span className="ml-1 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-slate-500">{r.grade}</span>}</div><div className="mt-1 flex flex-wrap gap-1">{!r.visited && <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-[10px] font-bold text-slate-400">미방문</span>}{r.workKinds.map((k) => <span key={k} className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${tones[k]}`}>{icons[k]} {WORK_LABELS[k]}</span>)}</div></div><div className="text-right text-xs text-slate-500">{r.machineCount > 0 && <div>{r.machineCount}대</div>}<div className="font-semibold">{hm(r.workKinds.reduce((n, k) => n + Number(r.minutes[k] || 0), 0))}</div></div></div>)}</div>;
