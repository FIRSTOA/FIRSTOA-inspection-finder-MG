  ...
  return <div className="space-y-4 pb-16">
    <section className="flex flex-col gap-3 rounded-xl bg-[#151A23] p-3 shadow-sm lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {(kind === "weekly" || period === "week" || period === "month" || period === "quarter" || period === "year") && <select value={year} onChange={(e) => { const y = Number(e.target.value); setYear(y); if (kind === "weekly" || period === "week") setSelectedDay(weeksInMonth(y, month)[0]?.start || selectedDay); }} className="rounded-lg border border-white/15 bg-white/10 px-3 py-2 text-sm font-semibold text-white outline-none transition focus:border-blue-400 [color-scheme:dark]">{Array.from({ length: 6 }, (_, i) => currentYear - 4 + i).map((y) => <option key={y} value={y}>{y}년</option>)}</select>}
        {(kind === "weekly" || period === "week" || period === "month") && <>
          <select value={month} onChange={(e) => { const m = Number(e.target.value); setMonth(m); if (kind === "weekly" || period === "week") setSelectedDay(weeksInMonth(year, m)[0]?.start || selectedDay); }} className="rounded-lg border border-white/15 bg-white/10 px-3 py-2 text-sm font-semibold text-white outline-none transition focus:border-blue-400 [color-scheme:dark]">{Array.from({ length: 12 }, (_, i) => i + 1).map((m) => <option key={m} value={m}>{m}월</option>)}</select>
          {(kind === "weekly" || period === "week") && <select value={editWeek.start} onChange={(e) => setSelectedDay(e.target.value)} className="rounded-lg border border-white/15 bg-white/10 px-3 py-2 text-sm font-semibold text-white outline-none transition focus:border-blue-400 [color-scheme:dark]">{monthWeeks.map((w) => <option key={w.start} value={w.start}>{w.label} {shortDate(w.start)}~{shortDate(w.end)}</option>)}</select>}
        </>}
    ...
    {!loading && <>
      <section><div className="mb-3 flex items-end justify-between"><div><h3 className="text-lg font-black text-slate-950">외근</h3><p className="mt-0.5 text-xs font-semibold text-slate-400">방문·현장 업무 건수와 소요시간</p></div></div><div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-6">
      ...
      {kind === "daily" && period === "day" ? <div className="space-y-6"><div className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(420px,1fr)]">
        <div className="space-y-6">
          <section className="rounded-lg border border-slate-200 bg-white p-5"><h3 className="text-lg font-black text-slate-950">외근 영업 활동</h3><div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-3 2xl:grid-cols-6">{[
...
function PeriodBreakdown({ period, rows, officeLogs, start, end, year, month, quarter }: { period: Period; rows: VisitRow[]; officeLogs: OfficeLog[]; start: string; end: string; year: number; month: number; quarter: number }) {
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="mb-4"><h3 className="text-lg font-black text-slate-950">{title}</h3><p className="mt-0.5 text-xs font-semibold text-slate-400">{desc}</p></div>
    <div className="space-y-2 md:hidden">{summaries.map(({ bucket, summary, inside }) => <article key={`${bucket.start}-${bucket.end}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
      ...
      <div className="mt-3 grid grid-cols-4 gap-2 text-center">{[["방문", summary.visits], ["기기", summary.machines], ["점검", summary.count.inspection], ["AS", summary.count.as], ["납품", summary.count.delivery], ["PC·IT", summary.count.pc], ["미수", summary.count.misu], ["불만", summary.count.bulman]].map(([label, value]) => <div key={String(label)} className="rounded-lg bg-white px-1 py-2"><div className="text-[10px] font-bold text-slate-400">{label}</div><div className="mt-1 text-sm font-black text-slate-800">{value}</div></div>)}</div>
      <div className="mt-2 grid grid-cols-2 gap-2"><div className="rounded-lg bg-blue-50 px-3 py-2 text-xs font-bold text-blue-700">외근 {hm(summary.fieldMinutes)}</div><div className="rounded-lg bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700">내근 {hm(inside)}</div></div>
    </article>)}</div>
