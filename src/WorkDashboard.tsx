...
export default function WorkDashboard({ kind, author, focusDate }: { kind: "daily" | "weekly"; author: string; focusDate?: string | null }) {
  ...
  return <div className="space-y-4 pb-16">
    <section className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-3 shadow-sm lg:flex-row lg:items-center lg:justify-between">
      {kind === "daily" && <div className="grid w-full grid-cols-5 gap-1 rounded-md bg-slate-100 p-1 lg:w-auto">{periodTabs.map(([p, label]) => <button key={p} onClick={() => setPeriod(p)} className={`rounded-full px-1 py-2 text-xs font-bold transition sm:px-4 sm:text-sm ${period === p ? "bg-white text-slate-950 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>{label}</button>)}</div>}
      <div className="flex flex-wrap items-center gap-2">
        {kind === "daily" && period === "day" && <input type="date" value={selectedDay} onChange={(e) => { setSelectedDay(e.target.value); setYear(Number(e.target.value.slice(0, 4))); setMonth(Number(e.target.value.slice(5, 7))); setQuarter(Math.ceil(Number(e.target.value.slice(5, 7)) / 3)); }} className="rounded-full border border-slate-300 bg-white transition hover:bg-slate-50 px-3 py-2 text-sm font-semibold" />}
        {(kind === "weekly" || period === "week" || period === "month" || period === "quarter" || period === "year") && <select value={year} onChange={(e) => { const y = Number(e.target.value); setYear(y); if (kind === "weekly" || period === "week") setSelectedDay(weeksInMonth(y, month)[0]?.start || selectedDay); }} className="rounded-full border border-slate-300 bg-white transition hover:bg-slate-50 px-3 py-2 text-sm font-semibold">{Array.from({ length: 6 }, (_, i) => currentYear - 4 + i).map((y) => <option key={y} value={y}>{y}년</option>)}</select>}
        {(kind === "weekly" || period === "week" || period === "month") && <>
          <select value={month} onChange={(e) => { const m = Number(e.target.value); setMonth(m); if (kind === "weekly" || period === "week") setSelectedDay(weeksInMonth(year, m)[0]?.start || selectedDay); }} className="rounded-full border border-slate-300 bg-white transition hover:bg-slate-50 px-3 py-2 text-sm font-semibold">{Array.from({ length: 12 }, (_, i) => i + 1).map((m) => <option key={m} value={m}>{m}월</option>)}</select>
          {(kind === "weekly" || period === "week") && <select value={editWeek.start} onChange={(e) => setSelectedDay(e.target.value)} className="rounded-full border border-slate-300 bg-white transition hover:bg-slate-50 px-3 py-2 text-sm font-semibold">{monthWeeks.map((w) => <option key={w.start} value={w.start}>{w.label} {shortDate(w.start)}~{shortDate(w.end)}</option>)}</select>}
        </>}
        {kind === "daily" && period === "quarter" && <div className="flex gap-1">{[1,2,3,4].map((q) => <button key={q} onClick={() => setQuarter(q)} className={`rounded-full px-4 py-2 text-sm font-bold ${quarter === q ? "bg-slate-900 text-white" : "border border-slate-300 bg-white text-slate-600"}`}>{q}분기</button>)}</div>}
    ...
    {!loading && <>
      <section><div className="mb-3 flex items-end justify-between"><div><h3 className="text-lg font-bold text-slate-900">외근</h3><p className="text-xs text-slate-400">방문·현장 업무 건수와 소요시간</p></div></div><div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><div className="inline-flex rounded-full bg-slate-900 transition hover:bg-slate-800 px-2 py-1 text-xs font-bold text-white">🏢 방문 거래처</div><div className="mt-4 flex items-end justify-between"><div className="text-2xl font-black text-slate-950">{sum.visits}<span className="ml-1 text-xs font-semibold text-slate-400">곳</span></div><div className="text-xs font-bold text-slate-500">기기 {sum.machines}대</div></div></div>
        {KINDS.map((k) => {
        ...
        <div className="grid gap-3 xl:grid-cols-3">
          {note.bottlenecks.map((item, i) => (
            <div key={i} className="rounded-lg border border-rose-100 bg-white p-4 shadow-sm">
              ...
              <label className="block text-xs font-black text-slate-500">병목현상<input value={item.title} onChange={(e) => onBottleneckChange(i, "title", e.target.value)} className="mt-1 w-full rounded-full border border-slate-200 bg-white transition hover:bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-800 outline-none focus:border-rose-300" /></label>
              <label className="mt-3 block text-xs font-black text-slate-500">원인<AutoGrowTextarea value={item.cause} onChange={(value) => onBottleneckChange(i, "cause", value)} rows={1} className="mt-1 w-full rounded-full border border-slate-200 bg-white transition hover:bg-slate-50 px-3 py-2 text-sm leading-6 text-slate-700 outline-none focus:border-rose-300" /></label>
...
function PeriodBreakdown({ period, rows, officeLogs, start, end, year, month, quarter }: { period: Period; rows: VisitRow[]; officeLogs: OfficeLog[]; start: string; end: string; year: number; month: number; quarter: number }) {
  ...
  const summaries = buckets.map((bucket) => ({
    bucket,
    summary: summarize(rows.filter((row) => inRange(row.workDate, bucket.start, bucket.end))),
    inside: officeMinutes(officeLogs.filter((office) => inRange(office.workDate, bucket.start, bucket.end))),
  }));
  return <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="mb-4"><h3 className="text-lg font-bold text-slate-900">{title}</h3><p className="text-xs text-slate-400">{desc}</p></div>
    <div className="space-y-2 md:hidden">{summaries.map(({ bucket, summary, inside }) => <article key={`${bucket.start}-${bucket.end}`} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
      <div className="flex items-center justify-between"><div className="font-black text-slate-900">{bucket.label}</div><div className="text-[10px] font-semibold text-slate-400">{bucket.sub}</div></div>
      <div className="mt-3 grid grid-cols-4 gap-2 text-center">{[["방문", summary.visits], ["기기", summary.machines], ["점검", summary.count.inspection], ["AS", summary.count.as], ["납품", summary.count.delivery], ["PC·IT", summary.count.pc], ["미수", summary.count.misu], ["불만", summary.count.bulman]].map(([label, value]) => <div key={String(label)} className="rounded-md bg-white px-1 py-2"><div className="text-[10px] font-bold text-slate-400">{label}</div><div className="mt-1 text-sm font-black text-slate-800">{value}</div></div>)}</div>
      <div className="mt-2 grid grid-cols-2 gap-2"><div className="rounded-md bg-blue-50 px-3 py-2 text-xs font-bold text-blue-700">외근 {hm(summary.fieldMinutes)}</div><div className="rounded-md bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700">내근 {hm(inside)}</div></div>
    </article>)}</div>
    <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[900px] text-left"><thead><tr className="border-b border-slate-200 text-xs text-slate-400"><th className="px-3 py-3">구분</th><th className="px-3 py-3 text-right">방문</th><th className="px-3 py-3 text-right">기기</th><th className="px-3 py-3 text-right">점검</th><th className="px-3 py-3 text-right">AS</th><th className="px-3 py-3 text-right">납품</th><th className="px-3 py-3 text-right">PC·IT</th><th className="px-3 py-3 text-right">미수</th><th className="px-3 py-3 text-right">불만</th><th className="px-3 py-3 text-right">외근시간</th><th className="px-3 py-3 text-right">내근시간</th></tr></thead><tbody>{summaries.map(({ bucket, summary, inside }) => <tr key={`${bucket.start}-${bucket.end}`} className="border-b border-slate-100 last:border-0 hover:bg-slate-50"><td className="px-3 py-4 font-bold text-slate-800">{bucket.label} {bucket.sub && <span className="ml-1 text-[10px] font-normal text-slate-400">{bucket.sub}</span>}</td><td className="px-3 py-4 text-right font-semibold">{summary.visits}</td><td className="px-3 py-4 text-right">{summary.machines}</td><td className="px-3 py-4 text-right">{summary.count.inspection}</td><td className="px-3 py-4 text-right">{summary.count.as}</td><td className="px-3 py-4 text-right">{summary.count.delivery}</td><td className="px-3 py-4 text-right">{summary.count.pc}</td><td className="px-3 py-4 text-right">{summary.count.misu}</td><td className="px-3 py-4 text-right">{summary.count.bulman}</td><td className="px-3 py-4 text-right font-semibold text-blue-700">{hm(summary.fieldMinutes)}</td><td className="px-3 py-4 text-right font-semibold text-emerald-700">{hm(inside)}</td></tr>)}</tbody></table></div>
  </section>;
}
