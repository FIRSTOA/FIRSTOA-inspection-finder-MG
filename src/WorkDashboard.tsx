    ...
    {!loading && <>
      ...
      {kind === "daily" && period === "day" ? <div className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(420px,1fr)]">
        ...
        <section className="order-1 space-y-5">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4"><h3 className="text-lg font-bold text-slate-900">이번 주 계획과 결과</h3><p className="text-xs text-slate-400">핵심 흐름만 세 칸으로 정리합니다.</p></div><div className="grid gap-3 lg:grid-cols-3">{([['thisWeekGoal','이번 주 목표','🎯'],['thisWeekResult','이번 주 결과','✅'],['nextWeekGoal','다음 주 목표','➡️']] as [WeeklyTextKey,string,string][]).map(([k,label,icon])=><button key={k} onClick={()=>setEditField(k)} className="min-h-44 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left transition hover:border-blue-300 hover:bg-white"><div className="text-sm font-bold text-slate-800">{icon} {label}</div><div className="mt-3 line-clamp-6 whitespace-pre-wrap text-sm leading-6 text-slate-600">{note[k] || <span className="text-slate-300">클릭해서 작성하세요.</span>}</div><div className="mt-3 text-xs font-bold text-blue-600">{note[k] ? "전체보기·수정" : "작성하기"}</div></button>)}</div></div>
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><div><h3 className="text-lg font-bold text-slate-900">회고·성장 기록</h3><p className="text-xs text-slate-400">내용은 카드로 읽고, 수정할 때만 큰 편집창이 열립니다.</p></div><button onClick={()=>setEditField("review")} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600">주간 총평 {note.review ? "보기" : "작성"}</button></div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{weeklyCards.map((c)=><button key={c.key} onClick={()=>setEditField(c.key)} className={`min-h-48 rounded-2xl p-4 text-left transition hover:ring-2 hover:ring-blue-200 ${c.tone}`}><div className="text-sm font-bold text-slate-800">{c.icon} {c.label}</div><div className="mt-3 line-clamp-6 whitespace-pre-wrap text-sm leading-6 text-slate-600">{note[c.key] || <span className="text-slate-300">작성된 내용이 없습니다.</span>}</div><div className="mt-3 text-xs font-bold text-slate-500">전체보기·수정 →</div></button>)}</div></div>
          <button onClick={saveWeekly} disabled={saving === "weekly"} className="w-full rounded-xl bg-[#172033] py-3.5 text-sm font-bold text-white disabled:opacity-50">{saving === "weekly" ? "저장 중…" : "주간 기록 저장"}</button>
        </section>
      </div>}
    </>}
    {editField && <WeeklyEditModal field={editField} note={note} onChange={setNoteField} onClose={()=>setEditField("")} />}
  </div>;
}

function WeeklyEditModal({ field, note, onChange, onClose }: { field: WeeklyTextKey; note: WeeklyNote; onChange: <K extends keyof WeeklyNote>(k: K, v: WeeklyNote[K]) => void; onClose: () => void }) {
  const meta: Record<WeeklyTextKey, [string,string]> = { thisWeekGoal:["이번 주 목표","이번 주에 달성하려는 목표를 작성하세요."], thisWeekResult:["이번 주 결과","달성 결과와 미진행 사유를 함께 작성하세요."], nextWeekGoal:["다음 주 목표","다음 주에 실행할 목표를 작성하세요."], review:["주간 총평","이번 주 전체 흐름을 정리하세요."], growth:["성장노트","상황·문제점·개선점·실행을 중심으로 작성하세요."], challenge:["새로운 도전·아이디어","떠오른 아이디어와 실행 방법을 작성하세요."], special:["특이사항","업무상 공유할 특이사항을 작성하세요."], learning:["배운 점","새로 배우거나 익힌 내용을 작성하세요."], request:["지원·요청·건의사항","필요한 지원과 건의사항을 작성하세요."], praise:["칭찬","함께한 동료의 좋은 점을 작성하세요."] };
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm" onClick={onClose}><div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl" onClick={(e)=>e.stopPropagation()}><div className="flex items-center justify-between border-b border-slate-100 px-6 py-5"><div><h3 className="text-xl font-bold text-slate-900">{meta[field][0]}</h3><p className="mt-1 text-xs text-slate-400">{meta[field][1]}</p></div><button onClick={onClose} className="rounded-xl bg-slate-100 px-3 py-2 text-sm font-bold text-slate-500">닫기 ✕</button></div><textarea autoFocus value={String(note[field])} onChange={(e)=>onChange(field,e.target.value)} placeholder="내용을 입력하세요." className="min-h-[55vh] flex-1 resize-none p-6 text-base leading-8 text-slate-700 outline-none"/><div className="border-t border-slate-100 p-4"><button onClick={onClose} className="w-full rounded-xl bg-[#172033] py-3 text-sm font-bold text-white">작성 완료</button></div></div></div>;
}
