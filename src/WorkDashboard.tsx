...
import {
  ...
  getWeeklyNote, kstDate, saveOfficeLog, saveWeeklyNote, setVisitsCancelledBySource, weekRange,
...
export default function WorkDashboard({ kind, author }: { kind: "daily" | "weekly"; author: string }) {
  ...
  return <div className="space-y-4 pb-16">
    <section className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-3 shadow-sm lg:flex-row lg:items-center lg:justify-between">
      {kind === "daily" && <div className="grid w-full grid-cols-5 gap-1 rounded-md bg-slate-100 p-1 lg:w-auto">{periodTabs.map(([p, label]) => <button key={p} onClick={() => setPeriod(p)} className={`rounded px-1 py-2 text-xs font-bold transition sm:px-4 sm:text-sm ${period === p ? "bg-white text-slate-950 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>{label}</button>)}</div>}
