    ...
    {!loading && <>
      ...
      {kind === "daily" && period === "day" ? <div className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(420px,1fr)]">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-5"><h3 className="text-lg font-bold text-slate-900">주간 목표·성장 기록</h3><p className="text-xs text-slate-400">목표는 체크리스트로 관리하고, 경험과 배운 점은 분기 카드의 자료로 쌓입니다.</p></div>
          <GoalChecklist note={note} onChange={setNote} />
          <label className="mt-5 block"><span className="text-xs font-bold text-slate-600">주간 총평</span><textarea value={note.review} onChange={(e) => setNoteField("review", e.target.value)} rows={3} className="mt-1 w-full resize-y rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm outline-none focus:border-slate-400 focus:bg-white" /></label>
          <div className="mt-5 grid gap-4 md:grid-cols-2">{([['growth','💡 성장노트'],['challenge','🎯 새로운 도전·아이디어'],['special','🧡 특이사항'],['learning','🧡 배운 점'],['request','🤝 지원·요청·건의사항'],['praise','🤝 칭찬']] as const).map(([k, label]) => <label key={k}><span className="text-xs font-bold text-slate-600">{label}</span><textarea value={note[k]} onChange={(e) => setNoteField(k, e.target.value)} rows={5} className="mt-1 min-h-32 w-full resize-y rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm leading-relaxed outline-none focus:border-slate-400 focus:bg-white" /></label>)}</div>
          <button onClick={saveWeekly} disabled={saving === "weekly"} className="mt-5 w-full rounded-xl bg-[#172033] py-3 text-sm font-bold text-white disabled:opacity-50">{saving === "weekly" ? "저장 중…" : "주간 기록 저장"}</button>
        </section>
      </div>}
    </>}
  </div>;
...
function GoalChecklist({ note, onChange }: { note: WeeklyNote; onChange: (n: WeeklyNote) => void }) {
  const add = () => onChange({ ...note, goalItems: [...note.goalItems, { id: crypto.randomUUID(), title: "", target: "", result: "", status: "todo", reason: "", nextAction: "" }] });
  const set = (id: string, patch: Partial<WeeklyNote["goalItems"][number]>) => onChange({ ...note, goalItems: note.goalItems.map((g) => g.id === id ? { ...g, ...patch } : g) });
  return <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-center justify-between"><div><div className="text-sm font-bold text-slate-800">이번 주 목표 체크리스트</div><div className="text-[11px] text-slate-400">완료·실패·이월 상태와 다음 행동을 남기세요.</div></div><button onClick={add} className="rounded-lg bg-slate-800 px-3 py-2 text-xs font-bold text-white">+ 목표 추가</button></div>
    <div className="mt-3 space-y-3">{!note.goalItems.length && <button onClick={add} className="w-full rounded-xl border border-dashed border-slate-300 py-6 text-sm text-slate-400">첫 목표를 추가하세요</button>}{note.goalItems.map((g, i) => <div key={g.id} className="rounded-xl border border-slate-200 bg-white p-3"><div className="grid gap-2 md:grid-cols-[32px_1fr_120px_120px_36px] md:items-center"><span className="text-center text-xs font-bold text-slate-300">{i + 1}</span><input value={g.title} onChange={(e) => set(g.id, { title: e.target.value })} placeholder="목표 내용" className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold" /><input value={g.target} onChange={(e) => set(g.id, { target: e.target.value })} placeholder="목표 수치" className="rounded-lg border border-slate-200 px-3 py-2 text-sm" /><select value={g.status} onChange={(e) => set(g.id, { status: e.target.value as typeof g.status })} className="rounded-lg border border-slate-200 px-2 py-2 text-sm"><option value="todo">예정</option><option value="doing">진행중</option><option value="done">완료</option><option value="failed">실패</option><option value="carried">다음 주 이월</option></select><button onClick={() => onChange({ ...note, goalItems: note.goalItems.filter((x) => x.id !== g.id) })} className="text-slate-300 hover:text-rose-500">✕</button></div><div className="mt-2 grid gap-2 md:grid-cols-3"><input value={g.result} onChange={(e) => set(g.id, { result: e.target.value })} placeholder="결과" className="rounded-lg border border-slate-200 px-3 py-2 text-xs" /><input value={g.reason} onChange={(e) => set(g.id, { reason: e.target.value })} placeholder={g.status === "failed" ? "실패 사유 (필수)" : "미진행 사유"} className="rounded-lg border border-slate-200 px-3 py-2 text-xs" /><input value={g.nextAction} onChange={(e) => set(g.id, { nextAction: e.target.value })} placeholder="다음 행동" className="rounded-lg border border-slate-200 px-3 py-2 text-xs" /></div></div>)}</div>
  </div>;
}
