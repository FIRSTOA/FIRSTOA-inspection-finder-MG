...
// 데이터 통합층 전체 카테고리 — 통합시트 탭·콘솔(데이터 통합 웹앱)·동기화·메뉴 기준. 12개 모두 유지.
const CATEGORIES = ['불만', '미수', '초과', 'PC확장성', '복합기확장성', 'AS', '점검', '업체정보', '임대현황표', '재계약', '초과업체조정', '해지방어', '경영지원'];

// 검색/인덱스 전용 카테고리 — 통합서칭 웹앱(프론트)에 노출되는 9개.
// 임대현황표·해지방어·경영지원은 "검색에서만" 제외하고, 위 CATEGORIES(12개)는 그대로라
// 데이터 통합 웹앱(콘솔)·마스터 탭·동기화에는 계속 존재한다.
const SEARCH_CATEGORIES = ['불만', '미수', '초과', 'PC확장성', '복합기확장성', 'AS', '점검', '업체정보', '재계약'];
...
const CONFIG = {
  불만: {
    ...
    headerRow: 2,
    vendorCol: '업체명',
    metaFields: { date: '방문일', region: '지역' },
    // 시트(기존양식 16) + 카톡 /불만접수(신양식) 합집합. 시트헤더명과 정확히 일치해야 시트분이 컬럼 매핑됨.
    // dedupKeyFields 없음 → 전체(원문 포함) 기준 고유키. 시트분/카톡분 한 테이블(bulman)에 공존.
    displayCols: [
      '날짜', '작성자', '지역', '등급', '업체명',
      '거래처담당자', '거래처연락처', '담당자', '기종',
      '접수/처리',
      '불만유형', '불만정도', '불만항목', '고객감정상태',
      '불만내용', '불편내용', '고객요청사항',
      '방문일', '처리자', '조치내용', '현장고객반응',
      'AI_불만유형', 'AI_불만항목',
      '외부원인', '내부원인',
  ...
  미수: {
    ...
    sheets: ['수도권A_이윤아', '수도권B_박수민', '수도권C_박지은', '수도권D_김슬기', '수도권E_김슬기'],
    headerRow: 3,
    vendorCol: '거래처명',
  ...
  // 점검 — 카톡 양식 전용. 카톡 "구분:점검"(점검 포함) 메시지를 규칙 파싱해
  // 마스터의 "점검" 탭에 직접 적재(원본 시트 없음). 흐름은 AS와 같으나 적재처가 통합탭이다.
  점검: {
    kakaoOnly: true,
    sheets: [],
    vendorCol: '업체명',
  ...
  // 신양식 /계약갱신 (■섹션). AI가 displayCols 기준으로 추출. 구 6컬럼은 Supabase에 legacy로 잔존.
  재계약: {
    kakaoOnly: true,
    sheets: [],
    vendorCol: '업체명',
    metaFields: { date: '계약종료일', region: '지역' },
    displayCols: [
      '작성자', '지역', '등급', '업체명', '담당자', '기종', '계약종료일',
      '갱신상태', '갱신위험도',
      '고객반응현황', '불편요구', '업무변화', '타사비교',
      '월렌탈료', '사용량초과', '기존혜택',
      '제안일자', '제안조건', '설명내용', '제안고객반응',
      '외부원인', '내부원인',
      '갱신방향', '제시선택지', '내부협의',
      '후속담당자', '기한', '다음확인일',
      '최종상태', '관리포인트', '원문'
  ...
  // 재계약방의 "/1년 누적 초과 업체 조정" 폼 — 재계약과 같은 방이지만 제목으로 갈라 별도 적재.
  초과업체조정: {
    kakaoOnly: true,
    sheets: [],
    vendorCol: '업체명',
    metaFields: { date: '방문일', region: '지역' },
    displayCols: ['방문일', '작성자', '지역', '업체명', '담당자', '기종', '현재조건', '누적결과', '제안', '고객반응', '할인조건변경', '진행상태', '원문']
  },
