...
function appendFieldSheetRow_(request) {
  ...
  if (dataStart && sheet.getLastRow() >= dataStart) {
    ...
    for (var mh = 0; mh < markerHeaders.length; mh++) {
      ...
      var markerVals = sheet.getRange(dataStart, markerCol, sheet.getLastRow() - dataStart + 1, 1).getDisplayValues();
      for (var mv = markerVals.length - 1; mv >= 0; mv--) {
        if (String(markerVals[mv][0]).trim() !== "") { if (dataStart + mv > lastFilled) lastFilled = dataStart + mv; break; }
      }
    }
    var candidate = lastFilled ? lastFilled + 1 : dataStart;
    if (candidate <= sheet.getMaxRows()) { row = candidate; reusedRow = true; }
  }
  if (!row) {
    var previousRow = Math.max(headerRow + 1, sheet.getLastRow());
    sheet.insertRowAfter(previousRow);
    row = previousRow + 1;
  }

  // 새로 삽입한 행에만 수식을 복사한다 — 재사용 행은 시트가 깔아둔 수식이 이미 살아있다.
  ...
  const scanCount = row - 1 - scanFrom + 1;
